...
for($i=2; $i<=5; $i++)
{
	...
	$orderTest = Mage::getModel('sales/order')->loadByIncrementId($orders[$j]['orders_id']);
    $itemsTest = $orderTest->getAllVisibleItems();
	
/*	 echo "<pre>";
	print_r($orderTest);
 echo "</pre>";*/

foreach ($itemsTest as $itemTest):

//echo $itemTest->getProductId();
   $productTest = Mage::getModel("catalog/product")->load($itemTest->getProductId()); //product id
  // echo "<pre>";
 // print_r($productTest->getOptions());echo "</pre>";
    foreach ($productTest->getOptions() as $_option) {
	...
	/*echo "<pre>";
		...
		/*	echo "<pre>";
			...
			echo $query11 = "INSERT INTO local_sareez_db.orders_products_attributes(orders_id, orders_products_id, products_options, products_options_values, options_values_price,price_prefix, flag)
				VALUES ('".$orders[$j]['orders_id']."', '".$itemTest->getProductId()."', '', '".$v->default_title."', '".$v->price."', '+', '1')";
				
				if(!mysql_query($query11,$local_conn))
				{
					echo $query11;
					//exit;
				}
...
endforeach;
	...
	$chkSorder = "SELECT * from local_sareez_db.orders WHERE orders_id ='".$orders[$j]['orders_id']."' ";
	...
	if(empty($orderExists))
	{
		...
		$orderSql = "INSERT into local_sareez_db.orders(orders_id, customers_id, customers_name, customers_company, customers_street_address, customers_suburb, customers_city, 
		...
		{
			$query1 = "Insert into local_sareez_db.orders_total VALUES ('','".$orders[$j]['orders_id']."','Sub-Total:','".$orders[$j]['currency'].$orders[$j]['pricing']['ot_subtotal']."', '".$orders[$j]['pricing']['base_ot_subtotal']."', 'ot_subtotal', 0, 1)";
			if(!mysql_query($query1, $local_conn))
			{
				echo $query1;
			}
			$query2 = mysql_query("Insert into local_sareez_db.orders_total VALUES ('','".$orders[$j]['orders_id']."','Shipping:','".$orders[$j]['currency'].$orders[$j]['pricing']['ot_shipping']."', '".$orders[$j]['pricing']['base_ot_shipping']."', 'ot_shipping', 0, 1)", $local_conn);
			$query3 = mysql_query("Insert into local_sareez_db.orders_total VALUES ('','".$orders[$j]['orders_id']."','Total:','".$orders[$j]['currency'].$orders[$j]['pricing']['ot_total']."', '".$orders[$j]['pricing']['base_ot_total']."', 'ot_total', 0, 1)", $local_conn);
			...
			{
				$orderProductsSql = "Insert into local_sareez_db.orders_products (orders_products_id,orders_id,products_id,products_model,products_name,products_image,normal_product_price,products_price,final_price,products_quantity,flag,products_status,products_express) VALUES ('".$product['orders_products_id']."', '".$orders[$j]['orders_id']."', '".$product['products_id']."',
			...
			if(isset($order['attributes']))
			{
				...
				{
					$query5 = "Insert into local_sareez_db.orders_products_attributes(orders_id,orders_products_id,products_options,products_options_values,options_values_price,price_prefix
			...
			while($r = mysql_fetch_assoc($query6))
			{
				...
				echo "Test *****************************************************"; 
				echo $query7 = "INSERT INTO local_sareez_db.ims_stitching_master(products_model, orders_products_id, products_id, orders_id, sending_date,receiving_date,tailor_master_id,stitching_status_id, stitching_comments,receiving_status)
				VALUES ('".$r['sku']."','".$r['item_id']."', '".$r['product_id']."', '".$orders[$j]['orders_id']."', '".$r['sending_date']."', '".$r['reciving_date']."', '".$r['tailormaster_id']."', '".$r['stitchingstatus_id']."', '".$r['comment']."', '".$receiving_status."' )";
		...
		$queryTest18 = mysql_query($sqlTest18,$conn) or die('error');
			...
			//exit;
				...
				{
					//echo "query8";exit;
					$query9 = "INSERT INTO local_sareez_db.orders_status_history(orders_id, orders_status_id, date_added, customer_notified, comments, flag)
					VALUES ('".$orders[$j]['orders_id']."','".$r['status']."', '".$r['created_at']."', '".$r['is_customer_notified']."', '".$r['comment']."',1)";
