...
$metabox_video = array(
		"video_type" => array (
			...
			"default" 	=> "",
			"type" 		=> "text",
			"desc"      => __('Upload your image with "Add Media" post window, copy the url and paste it here.', 'silesia')
		),
		"video_id" => array (
			"name"		=> "video_id",
			"default" 	=> "",
...
function video_meta_box_content() {
	...
	echo "\t".'<thead>';
	echo "\t".'<tr>';
	echo "\t".'<th class="left">'.__('Video Type', 'silesia').'</th>';
	echo "\t".'<th>'.__('Video ID' ,'silesia').'</th>';
	echo "\t".'</tr>';
	echo "\t".'</thead>';
	echo "\t".'<tr>';
	...
	echo "\t".'</tr>';
	echo '</table></div>'."\n\n";
	echo '<p>'.__('Select Video Type and insert Video ID', 'silesia').' (e.g. http://www.youtube.com/watch?v=<strong>Y2HIK1lgb3U</strong>).</p>'."\n\n";
...
function video_meta_box() {
	add_meta_box('video-settings',$GLOBALS['natty_themename'].' '.__('Video Settings', 'silesia'),'video_meta_box_content','post','normal');
	add_meta_box('video-settings',$GLOBALS['natty_themename'].' '.__('Video Settings', 'silesia'),'video_meta_box_content','page','normal');
}
