...
function nattywp_most_commented( $no_posts = 15, $before = '<li>', $after = '</li>', $show_pass_post = false, $duration = '', $echo = true ) {
	...
	if ( $echo ) {
		if ( !empty($posts) ) {
			...
			$t_most_commented .= $before . __('None found', 'silesia') . $after;
		}
	...
	if (!is_single()) {
		...
		if($max_page > 1 || $always_show) {
			...
			if ($paged >= ($pages_to_show-1)) {
				echo '<a href="'.get_pagenum_link().'">'. __('&laquo; First', 'silesia').'</a>';
			}
			...
			if (($paged+$half_pages_to_show) < ($max_page)) {
				echo '<a href="'.get_pagenum_link($max_page).'">'. __('Last &raquo;', 'silesia').'</a>';
			}
			echo "</div> $after";
		}
	}
...
function nattywp_show_catwithdesc() {
	$getcats = get_categories('hierarchical=0&hide_empty=1');
				...
				echo '<li><a href="'.get_category_link($thecat->term_id).'" title="'.__('View Posts in','silesia').' &quot;'.$thecat->name.'&quot;: '.$thecat->description.'">'.$thecat->name.'</a>'.$thecat->description.'</li>';
				}
}
