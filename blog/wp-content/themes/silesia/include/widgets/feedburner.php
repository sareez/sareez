...
?>
			...
			?>
				...
				<form name="feedburnerform" class="search" id="searchforma" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $id; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
					<div class="field">
					<input type="text" name="email" value="<?php _e('Enter your e-mail address','silesia'); ?>" onfocus="if (this.value == '<?php _e('Enter your e-mail address','silesia'); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e('Enter your e-mail address','silesia'); ?>';}" />
                     </div>
					<input type="hidden" value="<?php echo $id; ?>" name="uri"/>
					<input type="hidden" name="loc" value="en_US"/>
					<input type="submit" value="<?php _e('Subscribe','silesia'); ?>" />
...
function feedburnerWidgetAdmin() {
	...
	echo '<p>
			<label for="feedburner_id">'.__('Your Feedburner ID:', 'silesia').'
...
function feedburnerWidget_register(){
	wp_register_sidebar_widget('feedburner-1',__('NattyWP Feedburner Subscription', 'silesia'), 'feedburnerWidget');
	wp_register_widget_control('feedburner-1',__('NattyWP Feedburner Subscription', 'silesia'), 'feedburnerWidgetAdmin', 100, 200);
}
