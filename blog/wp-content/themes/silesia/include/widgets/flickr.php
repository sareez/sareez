...
function flickrWidgetAdmin() {
	...
	echo '<p>
			<label for="flickr_title">'.__('Title:', 'silesia').'
			<input id="flickr_title" name="flickr_title" type="text" class="widefat" value="'.$settings['title'].'" /></label></p>';
			
	echo '<p>
			<label for="flickr_id">'.__('Flickr ID', 'silesia').' (<a href="http://www.idgettr.com">idGettr</a>):
			<input id="flickr_id" name="flickr_id" type="text" class="widefat" value="'.$settings['id'].'" /></label></p>';
	echo '<p>
			<label for="flickr_number">'.__('Number of photos:', 'silesia').'
...
function flickrWidget_register(){
	wp_register_sidebar_widget('flickr-1', __('NattyWP Flickr', 'silesia'), 'flickrWidget');
	wp_register_widget_control('flickr-1', __('NattyWP Flickr', 'silesia'), 'flickrWidgetAdmin', 300, 200);
}
