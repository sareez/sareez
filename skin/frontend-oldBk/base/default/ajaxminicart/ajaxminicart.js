...
//alert(res.cartsubtotal);										//$('mini-cart-total').update('$'+parseFloat(res.carttotal).toFixed(2)); 
...
$mj(function($) {   
    $mj('.header-cart .relative').hoverIntent({
        over: function() {           
            $mj(this).children('.cart-items-box').slideDown(300); 
            $mj('.cart-items-box').show();           
        },
        out: function() {            
            $mj(this).children('.cart-items-box').slideUp(300);           
            $mj('.cart-items-box').hide();
        }
    });    
    
     /* cart page delete operation  */
    $mj(".cart-item-box td.last a" ).click(function() {      
      // $(this).closest('tr').remove();
       $mj(this).closest('tr').hide('slow');
       var id ="li-"+$mj(this).attr('id');              
       if(id){            
            $mj("#"+id).hide('slow');            
       }         
    });
    $mj("li a.remove-btn" ).click(function() {      
       $mj(this).closest('li').hide('slow'); 
       var id ="item-"+$mj(this).attr('id');        
       if(id){
          $mj("#"+id).hide('slow'); 
       }
    });    
