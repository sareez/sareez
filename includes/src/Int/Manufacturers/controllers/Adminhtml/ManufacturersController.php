<?php
class Int_Manufacturers_Adminhtml_ManufacturersController extends Mage_Adminhtml_Controller_Action {
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('manufacturers')->__('Item does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				if(isset($_FILES['filename']['manufacturers_image']) && $_FILES['filename']['manufacturers_image'] != '') {
				try {	
					/* Starting upload */	
					...
					$uploader->save($path, $_FILES['filename']['manufacturers_image'] );
		        ...
		        //this way the name is saved in DB
	  			$data['manufacturers_image'] = $_FILES['filename']['manufacturers_image'];
			}
				
				$model->save(); 
				
				
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('manufacturers')->__('Item was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('manufacturers')->__('Unable to find item to save'));
        $this->_redirect('*/*/');
	}
