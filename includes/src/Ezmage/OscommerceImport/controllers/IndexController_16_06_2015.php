...
class Ezmage_OscommerceImport_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function step31Action()
    {
		...
		try {
			...
			$query = 'select categories.categories_id ,categories.categories_image, categories.parent_id, categories_description.categories_name from categories LEFT JOIN categories_description on categories_description.categories_id = categories.categories_id AND categories.categories_id > 389 and  categories_description.language_id = '.$conf_language_id;
			...
			$query = 'SELECT products_description.products_id, products_description.products_name FROM products_description LEFT JOIN products ON products_description.products_id = products.products_id
WHERE products_description.language_id ='.$conf_language_id.' AND products.`products_id`>171766 AND products.products_status =1';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if category exist
			...
			// Customers //
			///////////////
			$query = 'select customers_id from customers WHERE `customers_id` >57263';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
			...
			// Orders //
			///////////////
			$query = 'select orders_id from orders WHERE `orders_id` >123192';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if customers exist
    ...
    {			
		...
		try {			
			...
			$categories_array_new = $this->tep_get_subcategories($categories_array, 0, $readConnection);
			//echo "<pre>";
			//print_r($categories_array_new);
			//echo "</pre>";
			//die("tezt....");
			foreach($categories_array_new as $category_row){
    ...
    {				
		...
		try {			
			...
			foreach($results as $row) {
				...
				/*if ($product_osc['products_image5'] != ''){
					$allowimages5 = $this->getImageAllow($product_osc['products_image5']);
					if ($allowimages5){ 						
						$image_location5 = $this->getDownloadImage("product",$product_osc['products_image5']);				
						if ( file_exists($image_location5) ) {
							$product->addImageToMediaGallery($image_location5,array('thumbnail','small_image','image'),true,false);
						}
					}
				}
				
				if ($product_osc['products_image4'] != ''){
					$allowimages4 = $this->getImageAllow($product_osc['products_image4']);
					if ($allowimages4){ 						
						$image_location4 = $this->getDownloadImage("product",$product_osc['products_image4']);				
						if ( file_exists($image_location4) ) {
							$product->addImageToMediaGallery($image_location4,array('thumbnail','small_image','image'),true,false);
						}
					}
				}
				
				if ($product_osc['products_image3'] != ''){
					$allowimages3 = $this->getImageAllow($product_osc['products_image3']);
					if ($allowimages3){ 						
						$image_location3 = $this->getDownloadImage("product",$product_osc['products_image3']);				
						if ( file_exists($image_location3) ) {
							$product->addImageToMediaGallery($image_location3,array('thumbnail','small_image','image'),true,false);
						}
					}
				}*/
				
				if ($product_osc['products_image2'] != ''){
					$allowimages2 = $this->getImageAllow($product_osc['products_image2']);
					if ($allowimages2){ 						
						$image_location2 = $this->getDownloadImage("product",$product_osc['products_image2']);				
						if ( file_exists($image_location2) ) {
							$product->addImageToMediaGallery($image_location2,array('thumbnail','small_image','image'),true,false);
						}
					}
				}
				...
				if ($product_osc['products_image1'] != ''){
					$allowimages1 = $this->getImageAllow($product_osc['products_image1']);
					if ($allowimages1){ 						
						$image_location1 = $this->getDownloadImage("product",$product_osc['products_image1']);				
						if ( file_exists($image_location1) ) {
							$product->addImageToMediaGallery($image_location1,array('thumbnail','small_image','image'),true,false);
						}
					}
				}
