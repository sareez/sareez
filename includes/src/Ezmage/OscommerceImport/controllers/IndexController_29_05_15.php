...
WHERE products_description.language_id ='.$conf_language_id.' AND products.`products_id`>167361 AND products.products_status =1';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if category exist
			...
			// Customers //
			///////////////
			$query = 'select customers_id from customers WHERE `customers_id` >56730';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
			...
			// Customers //
			///////////////
			$query = 'select orders_id from orders WHERE `orders_id` >120945';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if customers exist
