...
WHERE products_description.language_id ='.$conf_language_id.' AND products.`products_id`>171623 AND products.products_status =1';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if category exist
			...
			// Customers //
			///////////////
			$query = 'select customers_id from customers WHERE `customers_id` >57251';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
			...
			// Orders //
			///////////////
			$query = 'select orders_id from orders WHERE `orders_id` >123159';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if customers exist
