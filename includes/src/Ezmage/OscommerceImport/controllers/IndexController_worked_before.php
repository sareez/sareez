...
class Ezmage_OscommerceImport_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {						
		...
		$importCategoryTotal =2;
		$importProductsTotal = 2;
    ...
    public function step2Action()
    {
		...
		if (($ezmage_products == 1) and ($ezmage_log == 1) ){
			$step2Status = 'ok';
...
WHERE products_description.language_id ='.$conf_language_id.' AND products.products_status =1';
			$results  = $_connection_remote->fetchAll($query);
			foreach($results as $row) {
				// check if category exist
    ...
    {				
		...
		try {			
			...
			foreach($results as $row) {
				...
				if($product_osc['products_express']==0){
						$products_express = 71;
				} else if($product_osc['products_express']==1){
						$products_express = 33;
				} else if($product_osc['products_express']==2){
						$products_express = 34;
				} else if($product_osc['products_express']==3){
						$products_express = 35;
				} else {
					$products_express = 71;	
				}
				...
				$product->setExpreesShippingAvailable($products_express);
		...
		try {			
			...
			foreach($results as $row) {
					...
					if (sizeof($customer_results) == 1){
						...
						/*echo "<pre>";
						print_r($customer_results);
						echo "</pre>";
						*/
						//$password = rand(1111111,9999999);
						$password = $customer_results[0]['customers_password'] ;
						//die("ysdysadyas...d.ad.asdasd");
