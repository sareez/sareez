    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($request->getAllItems()) {
            ...
            foreach ($request->getAllItems() as $item) {
                ...
                if($_product->getPrice() > 49.99)
                {
                    $totalShippingAmout = $totalShippingAmout + ($item->getQty() * 9);
                }
                else
                {
                    $totalShippingWeight = $totalShippingWeight + ($_product->getWeight() * $item->getQty());
                }
                /* Custom code end */
        ...
        $request->setPackageWeight($totalShippingWeight);
        /* Custom code end */
        ...
        {
            $rate['price'] = $rate['price'] + $totalShippingAmout;
        }
        /* Custom code end */
