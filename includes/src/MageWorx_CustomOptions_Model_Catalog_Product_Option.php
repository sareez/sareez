...
//                exit;
    ...
    public function saveOptions() {
        ...
        $helper = Mage::helper('customoptions');
        if (!$helper->isEnabled() || Mage::app()->getRequest()->getControllerName()!='catalog_product') {
        ...
        foreach ($options as $key => $option) {
            ...
            if ($helper->isSkuNameLinkingEnabled() && (!isset($option['scope']['title']) && $option['scope']['title']!=1) && (!isset($option['title']) || $option['title']=='') && $this->getGroupByType($option['type'])!=self::OPTION_GROUP_SELECT && $option['sku']) {
                $options[$key]['title'] = $helper->getProductNameBySku($option['sku'], $this->getProduct()->getStoreId());
            }
        }
        $this->setOptions($options);        
            ...
            foreach ($this->getData('values') as $key => $value) {
                if (isset($value['option_type_id'])) {
                    ...
                    if (isset($value['customoptions_qty']) && $helper->getProductIdBySku($value['sku'])==0) {
                        ...
                        if (substr($customoptionsQty, 0, 1)!='x' && substr($customoptionsQty, 0, 1)!='i' && !is_numeric($customoptionsQty)) $customoptionsQty='';
                            ...
                            if (isset($value['specials']) && is_array($value['specials'])) {
                                ...
                                if (count($specials)>0) {
                                    foreach ($specials as $special) {
                                        $specialData = array('option_type_price_id'=>$optionTypePriceId, 'customer_group_id'=>$special['customer_group_id'], 'price'=>floatval($special['price']), 'price_type'=>$special['price_type'], 'comment'=>trim($special['comment']));
    ...
    public function getProductOptionCollection(Mage_Catalog_Model_Product $product) {
        ...
        if (!$isProductEditPage) {
            ...
            foreach($collection as $key => $item) {
                // 0-Disable, 1-Visible, 2-Hidden, 3-Backend
            ...
            if ($product->getRequiredOptions() && Mage::app()->getRequest()->getControllerName()=='product') {
                if ($helper->isInventoryEnabled() && $helper->getOutOfStockOptions()==1) {
                    ...
                    foreach ($collection as $option) {
                        ...
                        foreach ($option->getValues() as $value) {
                            $customoptionsQty = $helper->getCustomoptionsQty($value->getCustomoptionsQty(), $value->getSku(), $product->getId(), $value, 0);
        ...
        $skuPriceLinkingEnabled = $helper->isSkuPriceLinkingEnabled();
        ...
        foreach($collection as $key => $option) {
            if ($this->getGroupByType($option->getType())==self::OPTION_GROUP_SELECT) {
                ...
                foreach ($values as $value) {
                    ...
                    if (!$skuPriceLinkingEnabled || !$helper->applyLinkedBySkuDataToOption($value, $value->getSku(), $product->getStore(), $product->getTaxClassId())) {
                        // add special_prices
                ...
                if ($skuPriceLinkingEnabled && $helper->applyLinkedBySkuDataToOption($option, $option->getSku(), $product->getStore(), $product->getTaxClassId())) {
    ...
    public function getValueById($valueId) {
        if (isset($this->_values[$valueId])) {
            ...
            if (!$value->getIsPriceCalculated()) {
                ...
                if ($product) {
                    $helper = Mage::helper('customoptions');
                    if (!$helper->isSkuPriceLinkingEnabled() || !$helper->applyLinkedBySkuDataToOption($value, $value->getSku(), $product->getStore(), $product->getTaxClassId())) {
                        // add special_prices
