    ...
    implements Mana_Filters_Interface_Filter
{
	...
	foreach($collection as $products) {
	    ...
	    {
		//$minPrice = $_product->getSpecialPrice();
		$_store = $_product->getStore();
		$minPrice = $_store->roundPrice($_store->convertPrice($_product->getSpecialPrice()));
	    }
	    else
	    {
		//$minPrice = $_product->getPrice();
		$_store = $_product->getStore();
		$minPrice = $_store->roundPrice($_store->convertPrice($_product->getPrice()));
	    }		    
	    break;
	}		
	$this->_minPriceInt = $minPrice;
	return $this->_minPriceInt;
	//return 0;
    }
