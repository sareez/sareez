    ...
    extends Emjainteractive_Advancedoptions_Block_Catalog_Product_View_Options_Abstract
{
    ...
    public function getValuesHtml()
    {
        ...
        if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN
            ...
            $select->setExtraParams('onchange="this.form.submit()"'.$extraParams);
            ...
            switch ($_option->getType()) {
                case Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO:
                    ...
                    if (!$_option->getIsRequire()) {
                        $selectHtml .= '<li><input type="radio" id="options_'.$_option->getId().'" class="'.$class.' product-custom-option" name="cart[' . $item->getId() . '][options]['.$_option->getId().']" onclick="this.form.submit()" value="" checked="checked" /><span class="label"><label for="options_'.$_option->getId().'">' . $this->__('None') . '</label></span></li>';
                    }
                    break;
		...
		else
		{
		 $style ="";   
                ...
                $selectHtml .= '<li>' .
                               '<input type="'.$type.'" class="'.$class.' '.$require.' product-custom-option" onclick="this.form.submit()" name="cart[' . $item->getId() . '][options]['.$_option->getId().']'.$arraySign.'" id="options_'.$_option->getId().'_'.$count.'" value="'.$_value->getOptionTypeId().'" ' . $checked . '/>' .
