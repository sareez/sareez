...
class Mage_Adminhtml_Sales_Order_ShipmentController extends Mage_Adminhtml_Controller_Sales_Shipment
{
     ...
     */
    public function addTrackAction_bkknaba()
    {
        try {
            $carrier = $this->getRequest()->getPost('carrier');
            $number  = $this->getRequest()->getPost('number');
            $title  = $this->getRequest()->getPost('title');
		    $unique_stitching_id  = $this->getRequest()->getPost('unique_stitching_id');
            ...
            if ($shipment) {
                $track = Mage::getModel('sales/order_shipment_track')
                    ...
                    ->setTitle($title)
					->setUniqueStitchingId($unique_stitching_id);
                $shipment->addTrack($track)
                    ->save();
    ...
    { 
		...
		$sku='';
		$unique_stitching_id='';
		for($x=0; $x<count($_REQUEST['sku']); $x++){
			$sku = $sku.",".$_REQUEST['sku'][$x];
		}
		
		for($x=0; $x<count($_REQUEST['unique_stitching_id']); $x++){
			$unique_stitching_id = $unique_stitching_id.",".$_REQUEST['unique_stitching_id'][$x];
		}
		$sku = ltrim($sku,",");
		$unique_stitching_id = ltrim($unique_stitching_id,",");
		
		//echo $sku."HHHHH".$unique_stitching_id;		
		//echo "<pre>"; print_r($_REQUEST); die;
        try {
            $carrier = $this->getRequest()->getPost('carrier');
            $number  = $this->getRequest()->getPost('number');
		    ...
		    //$unique_stitching_id  = $this->getRequest()->getPost('unique_stitching_id');
		    $unique_stitching_id  = $unique_stitching_id;
		    $sku  = $sku;
		    $tracking_date  = $this->getRequest()->getPost('tracking_date');
            ...
            if ($shipment) {
                $track = Mage::getModel('sales/order_shipment_track')
                    ...
                    ->setCarrierCode($carrier)
                    ->setTitle($title)
					->setShippingDate($tracking_date)
					->setUniqueStitchingId($unique_stitching_id)
					->setSku($sku)
					->setSendMailCustomer($sendMail);
