 ...
 *
 */
class Glace_Menumobile_Lib_Data_Form_Element_Grid_Columns
	extends Varien_Data_Form_Element_Text
{
	...
	public function getElementHtml()
	{
		...
		$html .= '</div>';
		$html .= '<div class="validation-advice" style="display:none;" id="' . $mainValidationId . '">' . Mage::helper('menumobile')->__('Sum of the grid units should be equal %s', $this->_gridUnitMax) . '</div>';
		...
		//Scripts
		if (!Mage::registry('glace_admin_jquery'))
		{
			$jqueryUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_JS) . 'glace/menumobile/jquery-for-admin.min.js';
			$html .= '<script type="text/javascript" src="' . $jqueryUrl . '"></script>';
			$html .= '<script type="text/javascript">Glacemenumobile_172.noConflict();</script>';
			Mage::register('glace_admin_jquery', 1);
		}
		$html .= '
		<script type="text/javascript">
		//<![CDATA[
			Glacemenumobile_172(function($) {
