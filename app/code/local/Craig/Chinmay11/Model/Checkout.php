...
class Craig_Chinmay_Model_Checkout extends Mage_Payment_Model_Method_Abstract {

    protected $_code  = 'Chinmay';
    ...
    public function getOrderPlaceRedirectUrl() {
        return Mage::getUrl('Chinmay/redirect');
    }
    ...
    public function getFormFields() {
        ...
        $ChinmayFields = array();
        $ChinmayFields['sid']					= $this->getSid();
        $ChinmayFields['lang']                  = $this->getLanguage();
        $ChinmayFields['purchase_step']         = 'payment-method';
        $ChinmayFields['merchant_order_id']		= $order_id;
        $ChinmayFields['email']					= $order->getData('customer_email');
        $ChinmayFields['first_name']			= $b->getFirstname();
        $ChinmayFields['last_name']				= $b->getLastname();
        $ChinmayFields['phone']					= $b->getTelephone();
        $ChinmayFields['country']				= $b->getCountry();
        $ChinmayFields['street_address']		= $b->getStreet1();
        $ChinmayFields['street_address2']		= $b->getStreet2();
        $ChinmayFields['city']					= $b->getCity();

        if ($country == 'US' || $country == 'CA') {
            $ChinmayFields['state']             = $b->getRegion();
        } else {
            $ChinmayFields['state']				= 'XX';
        }

        $ChinmayFields['zip']					= $b->getPosChinmayde();

        if ($a) {
            $ChinmayFields['ship_name']             = $a->getFirstname() . ' ' . $a->getLastname();
            $ChinmayFields['ship_country']			= $a->getCountry();
            $ChinmayFields['ship_street_address']	= $a->getStreet1();
            $ChinmayFields['ship_street_address2']	= $a->getStreet2();
            $ChinmayFields['ship_city']				= $a->getCity();
            $ChinmayFields['ship_state']			= $a->getRegion();
            $ChinmayFields['ship_zip']				= $a->getPosChinmayde();
            $ChinmayFields['sh_cost']				= $shipping;
            $ChinmayFields['sh_weight']				= $weight;
            $ChinmayFields['ship_method']			= $ship_method;
        }
        $ChinmayFields['2co_tax']				= $tax;
        $ChinmayFields['2co_cart_type']			= 'magento';
        $ChinmayFields['x_Receipt_Link_URL']    = Mage::getUrl('Chinmay/redirect/success', array('_secure' => true));
        $ChinmayFields['return_url'] = Mage::getUrl('Chinmay/redirect/cart', array('_secure' => true));
        $ChinmayFields['demo']					= $this->getDemo();
        $ChinmayFields['currency_code'] = $currency_code;

        //Check Integration mode
        $lineitem_total = $this->checkTotal();
        if ($lineitem_total != $amount) {
            $ChinmayFields['id_type']			= '1';
            $ChinmayFields['total']				= $amount;
            $ChinmayFields['cart_order_id']			= $order_id;
            $result = $productData + $taxData + $ChinmayFields;
        } else {
            $ChinmayFields['mode']		        	= '2CO';
            $result = $ChinmayFields + $lineitemData;
        }
