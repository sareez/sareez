...
class MageWorx_CustomOptions_Helper_Product_Configuration extends MageWorx_CustomOptions_Helper_Product_Configuration_Abstract {
    ...
    public function getOptionFormatPrice($model, $optionTotalQty = 1, $product, $quote) {
        ...
        if ($price!=0) {
            ...
            // option taxClassId
            $taxClassId = ($model->getIsSkuPrice() ? $model->getTaxClassId() : $product->getTaxClassId());
            
            // calculate tax
            if ($price<0) {
                $priceInclTax = $price;
            } elseif (Mage::helper('tax')->priceIncludesTax($store)) {
                $priceInclTax = $price;
                ...
                if ($option) {
                    ...
                    switch ($option->getType()) {
                        ...
                        case 'multiswatch':
                            if (isset($quoteItemOptionInfoBuyRequest['options'][$optionId])) {                                                                
                                ...
                                foreach ($option->getValues() as $key=>$value) {
                                    if (isset($quoteItemOptionInfoBuyRequest['options_'.$optionId.'_'.$value->getOptionTypeId().'_qty'])) $optionQty = intval($quoteItemOptionInfoBuyRequest['options_'.$optionId.'_'.$value->getOptionTypeId().'_qty']); else $optionQty = 1;
                                    if (!isset($quoteItemOptionInfoBuyRequest['options'][$optionId]) || in_array($value->getOptionTypeId(), $quoteItemOptionInfoBuyRequest['options'][$optionId])) {
                        ...
                        case 'multiple':
                            ...
                            $optionTotalQty = ($option->getCustomoptionsIsOnetime()?$optionQty:$optionQty*$qty);
                            foreach ($option->getValues() as $key=>$value) {                                
                                if (!isset($quoteItemOptionInfoBuyRequest['options'][$optionId]) || $value->getOptionTypeId()==$quoteItemOptionInfoBuyRequest['options'][$optionId]) {
                                    if ($value->getOrigTitle()) $value->setTitle($value->getOrigTitle()); else $value->setOrigTitle($value->getTitle());
                                    $value->setTitle($filter->filter(($optionTotalQty>1?$optionTotalQty.' x ':'') . $value->getTitle() . $this->getOptionFormatPrice($value, $optionTotalQty, $product, $item->getQuote())));
                                }
                                $optionValues[$key]=$value;
