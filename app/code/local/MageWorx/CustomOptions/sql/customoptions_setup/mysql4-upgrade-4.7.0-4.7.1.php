...
// set required_options = 1 flag to product
//$this->run("UPDATE `{$installer->getTable('catalog_product_entity')}` SET `has_options` = 1, `required_options` = 1 WHERE `required_options` = 0 AND `entity_id` IN (SELECT DISTINCT `product_id` FROM `{$installer->getTable('catalog_product_option')}` WHERE `is_require` = 1);");
