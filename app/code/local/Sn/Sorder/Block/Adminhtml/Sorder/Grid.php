...
/*	  	 $this->addColumn('comment', array(
	  ...
	  $this->addColumn('created_at', array(
	   ...
	   $this->addColumn('increment_order_id', array(
		  ...
		  'renderer' => 'Sn_Sorder_Block_Adminhtml_Sorder_Grid_Renderer_Orderid'
      ));
	...
	$sql = "select `allocationstatus_id`,`title` from `allocationstatus` where 1=1 order by `title` ASC";
	$rows = $connection->fetchAll($sql);
	$allocationstatus=array(); 
	for($x=0; $x<count($rows); $x++){
		$allocationstatus[$rows[$x]['title']] = $rows[$x]['title'];
	}
	...
	$this->addColumn('title', array(
			...
			'renderer' => 'Sn_Sorder_Block_Adminhtml_Sorder_Grid_Renderer_Allocationstatus'
		  ));
	  ...
	  $this->addColumn('tentative_date', array(
          'header'    => Mage::helper('sorder')->__('Tentative Date'),
          'align'     =>'left',
          'width'     => '50px',
          'index'     => 'tentative_date',
		  'type'  => 'datetime',
		  'renderer' => 'Sn_Sorder_Block_Adminhtml_Sorder_Grid_Renderer_Tentativedate'
...
$this->addColumn('stitchingstatus_id', array(
    ...
    $this->addColumn('exprees_shipping_available', array(
          ...
          'index'     => 'exprees_shipping_available',
		 'type'      => 'options',
		  'options'   => array('7 days' => '7 days', '10 days' => '10 days', '15 days' => '15 days', '25 days' => '25 days', 'Free Gift' => 'Free Gift'),
      ));	  
	...
	$connection = Mage::getSingleton('core/resource')->getConnection('core_read');
	$sql = "select `tailormaster_id`,`tailorname` from `tailormaster` where 1=1 order by `tailorname` ASC";
	$rows = $connection->fetchAll($sql);
	$tailormaster=array(); 
	for($x=0; $x<count($rows); $x++){
		$tailormaster[$rows[$x]['tailorname']] = $rows[$x]['tailorname'];
	}
		
	$this->addColumn('tailorname', array(
			'header'    => Mage::helper('sorder')->__('Tailor Name'),
			'align'     =>'left',
			'width'     => '50px',
			'index'     => 'tailorname',
			'type'      => 'options',
			'options'   => $tailormaster,
			//'renderer' => 'Sn_Sorder_Block_Adminhtml_Sorder_Grid_Renderer_Manufacturers'
		  ));
	...
	/*   $this->addColumn('shipping_status', array(
	  ...
	  */
	  ...
	  $this->addColumn('shipping_method', array(
          'header'    => Mage::helper('sorder')->__('Shipping Method'),
          'align'     =>'left',
          'width'     => '50px',
          'index'     => 'shipping_method',
		  'renderer' => 'Sn_Sorder_Block_Adminhtml_Sorder_Grid_Renderer_Shippingmethod'
      ));
...
$this->addColumn('payment_method', array(
          ...
          'index'     => 'payment_method',
		  'type'      => 'options',
		  'options'   => array('2Checkout (Visa, MasterCard, Amex, Discover, JCB, Diners Club, Debit Card, PayPal)' => '2Checkout (Visa, MasterCard, Amex, Discover, JCB, Diners Club, Debit Card, PayPal)', 'Bank Transfer Payment (Will take 2-3 days extra time to ship)' => 'Bank Transfer Payment (Will take 2-3 days extra time to ship)', 'Cash on Delivery - India Only' => 'Cash on Delivery - India Only', 'CC Avenue' => 'CC Avenue', 'Ccavenue MCPG Payment' => 'Ccavenue MCPG Payment', 'Ccavenue Payment' => 'Ccavenue Payment', 'Check / Money order' => 'Check / Money order', 'PayPal Website Payments Standard' => 'PayPal Website Payments Standard'),
      ));
