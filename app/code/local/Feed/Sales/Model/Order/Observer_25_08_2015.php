...
mysql_query($sqlship) or die("ERROR4::".mysql_error());
	...
	/*for($x=0; $x<count($skuArr); $x++){
	 ...
	 foreach ($result as $key => $val) {
		...
		$ss="SELECT SUM( qty_ordered ) as qtyordered, SUM( qty_shipped ) as qtyshipped FROM  `sales_flat_order_item` WHERE  `order_id` ='".$trackData[0]['order_id']."'";
		$qq = mysql_query($ss) or die("ERROR_QTY_SHIPPED::".mysql_error());
		$rr=mysql_fetch_array($qq);
		$qtyordered = (int)$rr['qtyordered'];
		$qtyshipped = (int)$rr['qtyshipped'];
		if($qtyordered > $qtyshipped){
			
		$qty_shipped_status='And the following remaining product(s) will be shipped very soon.<br><br>';
		$sql_ord="SELECT * FROM  `sales_flat_order_item` WHERE `order_id` ='".$trackData[0]['order_id']."'";
		$query_ord = mysql_query($sql_ord) or die("ERROR_QTY_SHIPPED::".mysql_error());
		while($row_ord=mysql_fetch_array($query_ord)){

			$qty_ordered = (int)$row_ord['qty_ordered'];
			$qty_shipped = (int)$row_ord['qty_shipped'];
			if($qty_ordered > $qty_shipped){
				$total=($qty_ordered-$qty_shipped);
