<?php
class Most_ViewPurchased_Block_ViewPurchased extends Mage_Core_Block_Template
{
	public function _prepareLayout()
    {
     ...
     public function getViewPurchased()     
     { 
        if (!$this->hasData('viewpurchased')) {
            $this->setData('viewpurchased', Mage::registry('viewpurchased'));
        }
        return $this->getData('viewpurchased');
...
Mage::getSingleton('catalog/product_status')
->addVisibleFilterToCollection($products14);
Mage::getSingleton('catalog/product_visibility')
...
$storeId    = Mage::app()->getStore()->getId();
// get today and last 30 days time
$today = time() - (13 * 24 * 60 * 60);;
$last = $today - (13 * 24 * 60 * 60);
...
// get today and last 30 days time
$today_bs14 = time() - (6 * 24 * 60 * 60);;
$last_bs14 = $today_bs14 - (60*60*24*7);
...
// get today and last 30 days time
$today_bs30 = time() - (13 * 24 * 60 * 60);;
$last_bs30 = $today_bs30 - (60*60*24*7);
