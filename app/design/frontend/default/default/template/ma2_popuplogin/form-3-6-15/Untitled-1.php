...
$connection = $coreResource->getConnection('core_read');
 ...
 $sql = "SELECT `value` , `entity_id` FROM `customer_entity_varchar` WHERE `attribute_id` =12";
...
->setWebsiteId(1)
->loadByEmail("duttachinmay87@gmail.com");


$hash = $customer->getData("password_hash");
$password = "123456";
echo "Hash (Password in database): ".$customer->getData("password_hash");
echo "<br/>";
...
$salt = $hashArr[1];
 ...
 $pass = md5($salt . $password);*/
		...
		var passval=document.forms["login-form"]["login[password]"].value;
		var fruitsVal = [<?php //echo $g;?>];
		var passhash = CryptoJS.MD5(passval);
		alert(passhash);
		var a1 = fruitsVal.indexOf(passhash);
		if(a1==-1){		
			alert('1');
			return false;
		}else{		
